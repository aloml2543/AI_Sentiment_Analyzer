...
class dic:
    data ={"i":{'POS':'PRP','SC':0},
          ...
          'really':{'POS':'RB','SC':'up'},
          'for':{'POS':'IN','SC':0},
          'have':{'POS':'VB','SC':0, 'IF': {'NEXT':{'done': {'POS':'VBP','SC':0}}}},
...
def make_tree(sentence): #Take a sentence and output an array of final 'S' only.
    ...
    while(len(output) != 1):#Normally, output length of 1 ('S') stops the merge process
        #Stop function when arrangement is made up of S only.
        temp = 0
        for token in output:
            if token[0] != 'S':
                temp += 1
    ...
    #If the output's length is 1 and the VP is left, it will be changed to S.
    if len(output) == 1 and output[0][0] == 'VP':
            output[0][0] = 'S'
            print(output)
